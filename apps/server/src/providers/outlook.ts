...
export class OutlookProvider extends BaseMailProvider {
   ...
   * 断言「请重新授权」会把人推去做完全不必要的设备码流程。
   */
  protected override describeFailure(cause: unknown, channel: 'imap' | 'smtp'): string {
    if (isAuthFailure(cause)) {
      return `Outlook ${channel.toUpperCase()} 认证被拒绝。若 access token 是刚刷新出来的，多为并发连接触发的瞬时限流，下一轮同步通常自行恢复；只有持续失败才说明 refresh token 已失效，需要用设备码重新授权`;
    }
