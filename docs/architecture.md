...
到期判定是 `accounts.last_synced_at + sync_interval_seconds`，间隔就是配置值本身 ——
**不再叠随机抖动**。抖动本来是拿来防账号扎堆的，但它防不住：扎堆的根因是所有账号相位相同，
±20% 的噪声只把那个尖峰摊宽 20%，该一起醒的还是一起醒；代价却是「设置里写 300 秒、
实际 240~360 秒」，用户没法预期，排查限流时也说不清到底多久同步一次。

相位改由**串行那一圈**产生，并由排期锚点保住：

- 账号的**第一次**后台同步以**完成时刻**为锚（`下次 = 现在 + 间隔`）。后台层是串行的，
  第 i 个账号的完成时刻天然比第一个晚 i×(同步耗时 + 账号间隔)，一次导入 29 个账号
  也会被这一圈自动摊到 4 分钟里。
- 此后每次以**上一次的到期时刻**为锚（`下次 = 上次到期 + 间隔`）。周期于是精确等于配置值，
  上面挣来的相位被永久保持。换成锚在完成时刻，每轮都会把周期悄悄加长一个同步耗时，
  29 个账号各漂各的，几小时后又糊回一坨。
- 落后超过一整个间隔时（同步比间隔还慢、或进程被挂起过）就地重起相位，
  否则「上次到期 + 间隔」一直落在过去，这个账号会被每一个 tick 都判成到期。
- 用户点的同步（第 2、3 层）一律以这一刻为锚：他刚拿到新数据，基线没理由紧接着再来一遍。
...
| 机制 | 位置 | 触发 | 后果 | 解除 |
| --- | --- | --- | --- | --- |
| 相位错开 | `SyncScheduler` | 首次后台同步 | 按串行完成时刻摊开，此后锚在网格上保持 | — |
...
五者不重叠：相位错开决定「什么时候轮到谁」，重试处理秒级抖动，冷却降的是频率，
strikes 判的是凭据，升级停的是调度。
